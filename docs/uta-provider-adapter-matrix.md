...
|---|---|---|---|
| `massive_live_trade_slices` | Replay fixture | Massive trades API by default; Polygon-compatible key/base URL accepted. Massive-style fields are normalized into the UTA contract. | `unavailable` with no live signal. |
...
| `massive_daily_bars` | Replay fixture | Massive aggregate bars by default when `MARKET_DATA_PROVIDER=massive`; existing provider chain remains available. | `stale` or `unavailable`; no B score if baseline cannot be built. |
